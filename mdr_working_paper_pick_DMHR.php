...
{    
  ...
  if($num == 1){
    $query = mysql_query("UPDATE tb_upload_logbook SET send_tindaklanjut = 1, status_tindaklanjut='YES' WHERE id_logbook ='$id_logbook'");
    if($query){
      header("Location: ./mdr_working_paper_kk.php?ntf=101");                                                  
    } else {
      echo "Updated Failed - Please contact your Administrator";
    }
  } else {
...
{    
  ...
  $query = mysql_query("INSERT INTO tb_parameter (id_logbook, hasil_review,tindak_lanjut,report,CHR,bobot,saran_perbaikan,tanggapan,target_date,komentar,tema_id,reviewer,dashboard) VALUES 
    (
    '".$id_logbook."',
    '".$hasil_review."',
    '".$tindak."',
    ...
    ");
  if($query){
    header("Location: ./mdr_working_paper_kk.php?ntf=$uid");                                                  
  } else {
    echo "Updated Failed - Please contact your Administrator";
  }
}
...
{    
  ...
  if($query_update){
    header("Location: ./mdr_working_paper_kk.php?ntf=$id_par");                                                  
  } else {
    echo "Updated Failed - Please contact your Administrator";
  }
}
...
<style type="text/css">
  .divTable {
    width: 100%;
    display: table;
    -webkit-box-shadow: 1px 1px 1px 1px 1px #fff;
    box-shadow: 1px 1px 1px 1px 1px #fff;
  }
  .divTableRow {
    width: 100%;
    height: 100%;
          ...
          <div class="col-xs-12">
            <div class="box-header">
              <ol class="breadcrumb">
                ...
                <li class="active"><b>Working Paper Pick DMHR</b></li>
              </ol>
            </div><!-- /.box-header -->
          </div>
                ...
                <table id="example1" class="table table-hover">
                  ...
                  <tbody>
                        ...
                        $result = mysqli_query($con,"SELECT tb_upload_logbook.id_logbook,tb_upload_logbook.date_time_log,tb_upload_logbook.kd_cabang,tb_upload_logbook.kanwil_nm,
                          ...
                          WHERE tb_upload_logbook.reviewer=".$_SESSION['userid']." AND tb_upload_logbook.send_tindaklanjut=0  AND tb_upload_logbook.status_tindaklanjut is NULL ORDER BY id_logbook
                          ");   
                          if(mysqli_num_rows($result)>0){
                            $no = 0;
                            while($row = mysqli_fetch_array($result))
                            {
                              // $tipe_ccr = 'Working Paper Kualitas Kredit';
                              // try {
                              ...
                              // if (in_array($tipe_ccr, $json)) {
                                ...
                                // echo "<td>" . "<a href='#' data-toggle='modal' data-target='#IUCREATENA$row[id_logbook]' title='Update Result'><span class='label bg-navy margin'><font style='font-size: 10px'>Informasi Umum</font></span></a>" . "</td>";
                                // echo "<td>" . "<a href='#' data-toggle='modal' data-target='#NOTAALLERT$row[id_logbook]' title='Update Result'><span class='label bg-navy margin'><font style='font-size: 10px'>Nota Allert</font></span></a>" . "</td>";
                                ...
                                // echo "<td>" . "<a href='#' data-toggle='modal' data-target='#DASHBOARD-KK$row[id_logbook]' title='Update Result'><span class='label bg-navy margin'><font style='font-size: 10px'>Dashboard</font></span></a>" . "</td>";
                                // echo "<td align= ''width='200'>
                                // <a href='include/template/working-paper/KK/mdr_ex_TP-KK.php?id=$row[id_logbook]' title='Template WP'><span class='label label-primary'><font style='font-size: 10px'>Template WP Kualitas Kredit</font></span></a><br><br>                                                 
                                // <a href='include/template/working-paper/KK/mdr_ex_result-KK.php?id=$row[id_logbook]' title='Result'><span class='label label-primary'><font style='font-size: 10px'>Result WP Kualitas Kredit</font></span></a><br><br>";
                                echo "<td align= ''width='200'>";
