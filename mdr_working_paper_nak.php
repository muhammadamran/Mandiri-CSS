...
{    
  ...
  if($query){
    header("Location: ./mdr_working_paper_nak.php?ntf=99");                                                  
  } else {
...
{    
  ...
  if($num == 1){
    ...
    if($query){
      header("Location: ./mdr_working_paper_nak.php?ntf=101");                                                  
    } else {
      echo "Updated Failed - Please contact your Administrator";
    }
  } else {
...
{    
  ...
  $query = mysql_query("INSERT INTO tb_parameter (id_logbook, hasil_review,tindak_lanjut,dashboard,report,KP,bobot,saran_perbaikan,tanggapan,target_date,komentar,tema_id,reviewer,status_ccr) VALUES 
    (
    '".$id_logbook."',
    '".$hasil_review."',
    '".$tindak."',
    ...
    ");
  if($query){
    header("Location: ./mdr_working_paper_nak.php?ntf=$uid");                                                  
  } else {
    echo "Updated Failed - Please contact your Administrator";
  }
}
...
{    
  ...
  if($query_update){
    header("Location: ./mdr_working_paper_nak.php?ntf=100=$id_par");                                                  
  } else {
    echo "Updated Failed - Please contact your Administrator";
  }
}
          ...
          <div class="col-xs-12">
            <div class="box-header">
              <ol class="breadcrumb">
                ...
                <li class="active"><b>Working Paper Kualitas NAK</b></li>
              </ol>
            </div><!-- /.box-header -->
          </div>
                ...
                <table id="example1" class="table table-hover">
                  ...
                  $result = mysqli_query($con,"SELECT tb_upload_logbook.id_logbook,tb_upload_logbook.date_time_log,tb_upload_logbook.kd_cabang,tb_upload_logbook.kanwil_nm,
                   ...
                   if(mysqli_num_rows($result)>0){
                    ...
                    {
                      $tipe_ccr = 'Working Paper Kualitas NAK';
                      try {
                      ...
                      if (in_array($tipe_ccr, $json)) {
                        // $limitku = $limitku+$row['LIMIT_LOG'];
                        // $badeku = $badeku+$row['BADE_LOG'];
                        $no++;
                        echo "<tr>";
                        ...
                        echo "<td>" . "<a href='#' data-toggle='modal' data-target='#DASHBOARD-NAK$row[id_logbook]' title='Update Result'><span class='label bg-navy margin'><font style='font-size: 10px'>Dashboard</font></span></a>" . "</td>";
                        echo "<td align= ''width='200'>
                        <a href='include/template/working-paper/NAK/mdr_ex_TP-NAK.php?id=$row[id_logbook]' title='Template WP NAK'><span class='label label-primary'><font style='font-size: 10px'>Template WP NAK</font></span></a><br><br>                                                  
                        <a href='include/template/working-paper/NAK/mdr_ex_result-NAK.php?id=$row[id_logbook]' title='Result WP NAK'><span class='label label-primary'><font style='font-size: 10px'>Result WP NAK</font></span></a><br><br>";
                      ...
                      include './modals/wp_dashboard-NAK.php';
